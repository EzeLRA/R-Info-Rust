use crate::lib::parser::processor::{Instruccion, Proceso, Variable};
...
impl Bot {
    ...
    fn process_ecuation(&mut self, ecuation: &str) {
        ...
        self.tokens = tokens;
        self.expression = self.build_expression_tree();
    }

    fn build_expression_tree(&self) -> Option<Expression> {
        let rpn_tokens = self.shunting_yard();
    ...
    fn shunting_yard(&self) -> Vec<TokenOp> {
        ...
        for token in tokens {
            match token {
                TokenOp::Num(_) | TokenOp::Var(_) => result.push(token),
                TokenOp::ParenLeft => stack_ops.push(token),
                TokenOp::ParenRight => {
    ...
    fn get_simplified_expression(&self) -> Option<String> {
    ...
    fn resolve_ecuation(&self, context: &HashMap<String, i32>) -> Result<i32, String> {
        if let Some(expr) = &self.expression {
            expr.evaluate(context)
        } else {
            Err("No hay expresión para evaluar".to_string())
        }
    }
    
    fn evaluate_with_defaults(&self, defaults: &HashMap<String, i32>) -> Result<i32, String> {
        self.resolve_ecuation(defaults)
    }
    
    fn get_expression_tree(&self) -> Option<Expression> {
        self.expression.clone()
    }
}

pub struct RobotMind {
    type_name : String,
    position : (u8, u8),
    //color : String,
    my_variables : Vec<Variable>,
    instructions : Vec<Instruccion>,
    //running : (bool, bool)
}   
pub struct Info {
    name_program : String,
    procedures : Vec<Proceso>,
//  robots_types : Vec<Unknown>,
//  robots_programs : Vec<Unknown>,
//  areas_robots : Vec<Unknown>
}
